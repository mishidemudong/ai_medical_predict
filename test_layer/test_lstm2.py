...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 11 17:20:06 2018
...
if __name__=='__main__': 
    ...
    print('y_test shape:',testY.shape) #(412L,) #预测未来数据 
    #predict_next(model,dataset,seq_len,10) 
    print('> Data Loaded. Compiling...') 
    #建模 
    model = build_model([1, lstmFirstLayer, lstmSecondLayer, 1],seq_len) 
    #模型训练 
    history = model.fit(trainX,trainY,batch_size=batchSize,epochs=nb_epoch,validation_split=0.05,verbose=1,shuffle=True) 
    #测试数据预测 
    ...
    print ('Test Score: %.2f RMSE' % (testScore)) #预测未来数据 
    next_predicted_list = predict_next(model,scaler,dataset,seq_len,future_predict_num) #结果展示 
    plot_results(testPredict,testY[0],next_predicted_list) #保存模型 
    hyperparams_name=str(seq_len)+"-"+str(lstmFirstLayer)+"-"+str(lstmSecondLayer)+"-"+str(testScore) 
    model.save(os.path.join( 'MODEL', '{}_cont.h5'.format(hyperparams_name)), overwrite=True)
